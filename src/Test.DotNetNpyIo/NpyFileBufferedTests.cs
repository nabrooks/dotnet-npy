...
{
    public class NpyFileBufferedTests
    {
        ...
        public NpyFileBufferedTests(ITestOutputHelper testOutputHelper)
        {
        ...
        public void ReadSequential2DFloat(string filePath)
        {
            ...
            var npyFile = NpyFileBuffered<float>.Open(fileInfo);
        ...
        {
            var npyFile = NpyFileBuffered<float>.Create(new FileInfo($"float_{ni}_{nj}__{ShortGuid.NewGuid()}.npy"), new[] { ni, nj });
        ...
        public void CreateWriteSequentialReadSequential3DInt32(int ni, int nj, int nk)
        {
            ...
            var file = NpyFileBuffered<int>.Create(fileInfo, new int[] { ni, nj, nk }, true, false);
            ...
            int counter = 0;
            file = NpyFileBuffered<int>.Open(fileInfo);
            for (int i = 0; i < file.Shape[0]; i++)
                for (int j = 0; j < file.Shape[1]; j++)
                    for (int k = 0; k < file.Shape[2]; k++)
                    {
        ...
        public void CreateWriteSequentialReadSequential3DFloat(int ni, int nj, int nk)
        {
            ...
            var file = NpyFileBuffered<float>.Create(fileInfo, new int[] { ni, nj, nk }, true, false);
            ...
            float counter = 0;
            file = NpyFileBuffered<float>.Open(fileInfo);
            for (int i = 0; i < file.Shape[0]; i++)
                for (int j = 0; j < file.Shape[1]; j++)
                    for (int k = 0; k < file.Shape[2]; k++)
                    {
        ...
        public void CreateWriteSequentialReadSequential4DFloat(int ni, int nj, int nk, int nl)
        {
            ...
            var file = NpyFileBuffered<float>.Create(fileInfo, new int[] { ni, nj, nk, nl }, true, false);
        ...
        public void CreateWriteSequentialReadSequential5DDouble(int ni, int nj, int nk, int nl, int nm)
        {
            ...
            var file = NpyFileBuffered<Double>.Create(fileInfo, new int[] { ni, nj, nk, nl, nm }, true, false);
            for (int i = 0; i < file.Shape[0]; i++)
                for (int j = 0; j < file.Shape[1]; j++)
                    for (int k = 0; k < file.Shape[2]; k++)
                        for (int l = 0; l < file.Shape[3]; l++)
            ...
            file.Dispose();
            file = NpyFileBuffered<Double>.Open(fileInfo);
            int counter = 0;
            for (int i = 0; i < file.Shape[0]; i++)
                for (int j = 0; j < file.Shape[1]; j++)
                    for (int k = 0; k < file.Shape[2]; k++)
        ...
        public void CreateWriteSequentialReadRange3DDouble(int ni, int nj, int nk)
        {
            ...
            var npyFile = NpyFileBuffered<double>.Create(new FileInfo(fileName), new int[] { ni, nj, nk }, true, false);
        ...
        public void SuperLongMMapFileAccess(long ni)
        {
            ...
            var npyArrayFile = NpyFileBuffered<float>.Create(npyImplFileInfo, ni);
        ...
        public void CreateWriteRangeReadRange1DFloat(int ni)
        {
            ...
            var newImplFile = NpyFileBuffered<float>.Create(newImplFileInfo, new int[] { ni }, true, false);
            ...
            newImplFile = NpyFileBuffered<float>.Open(newImplFileInfo);
        ...
        public void CompareImplementationsRead(int ni, int nj, int nk)
        {
            ...
            var newImplFile = NpyFileBuffered<float>.Create(newImplFileInfo, new int[] { ni, nj, nk }, true, false);
        ...
        public void CreateWriteSequentialRead2DSliceFrom3DDouble(int ni, int nj, int nk)
        {
            ...
            var npyFile = NpyFileBuffered<double>.Create(testFileInfo, new int[] { ni, nj, nk });
            ...
            npyFile = NpyFileBuffered<double>.Open(testFileInfo);
            double[][] arr;
        ...
        public void CreateWriteSequentialRead2DSliceFrom3DFloat(int ni, int nj, int nk)
        {
            ...
            var npyFile = NpyFileBuffered<float>.Create(testFileInfo, shape, true, isFortranOrder, true);
            ...
            npyFile = NpyFileBuffered<float>.Open(testFileInfo);
            float[][] arr;
