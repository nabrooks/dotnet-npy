...
namespace DotNetNpyIo
{
    public class LittleEndianBinaryReader : BinaryReader, IBinaryReader
    {
        ...
        public unsafe double ReadIbmDouble()
        {
            ...
            fixed (byte* bptr = buffer)
            {
                ...
                if (fconv != 0)
                {
                    ...
                    if (fmant == 0)
                    {
                        fconv = 0; // guard: avoid the infinite normalisation loop when fmant==0
                    }
                    else
                    {
        ...
        public unsafe float ReadIbmSingle()
        {
            ...
            fixed (byte* bptr = buffer)
            {
                int fconv = IbmFloat.IbmBitsToIeeeBits(((int*)bptr)[0]);
                return *((float*)&fconv);
            }
        }
        ...
        public unsafe float[] ReadIbmSingles(int count)
        {
            ...
            fixed (byte* bptr = buffer)
            {
                ...
                {
                    int fconv = IbmFloat.IbmBitsToIeeeBits(((int*)bptr)[i]);
                    copy[i] = *((float*)&fconv);
                }
                return copy;
            }
